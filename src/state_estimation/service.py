...
class StateEstimationService:
    ...
    def on_feature_received(self, feature_data: FeatureData):
        ...
        result = FocusResultData(
            ...
            session_id=session_id,
            head_pose_score=100.0 - scores.get("head_pose", 0.0),
            eye_score=100.0 - scores.get("eye", 0.0),
            yawn_score=100.0 - scores.get("yawn", 0.0),
            distance_score=100.0 - scores.get("distance", 0.0),
    ...
    def _debug_scores(scores: dict, is_force_zero: bool, is_over_threshold: bool, warn_candidates: tuple):
        ...
        lines = [
            ...
            f"  --- 低头计时器 (class only) ---",
            f"  down : {scores.get('temporal_down_s', 0):.1f}s / {scores.get('_t_max_s', 30.0):.0f}s",
            f"  gap  : {scores.get('temporal_gap_s', 0):.1f}s / {scores.get('_gap_tol_s', 3.0):.0f}s",
            "  --- 融合结果 ---",
